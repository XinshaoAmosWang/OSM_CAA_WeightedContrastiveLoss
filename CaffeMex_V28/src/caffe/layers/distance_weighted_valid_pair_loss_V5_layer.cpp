...
#include "caffe/layers/distance_weighted_valid_pair_loss_V5_layer.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe {

template <typename Dtype>
void DistanceWeightedValidPairLossV5Layer<Dtype>::LayerSetUp(
...
template <typename Dtype>
void DistanceWeightedValidPairLossV5Layer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void DistanceWeightedValidPairLossV5Layer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void DistanceWeightedValidPairLossV5Layer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    for(int i = 0; i < pair_num; i++){
      ...
      if( static_cast<int>(pair_label.cpu_data()[i]) ){
        ...
        Dtype weight = gaussianfunc(dist, sigma, mean);
        //
        
        sum_similar_dw += (weight * pair_score[i]);
      }
      else{
        //dissimilar pairs
        Dtype dist = sqrt(auxiLayer->dist_sq_.cpu_data()[i]);
    ...
    for(int i = 0; i < pair_num; i++){
      ...
      if( static_cast<int>(pair_label.cpu_data()[i]) ){
        ...
        pair_score[i] = 0.5 * (weight * pair_score[i]) / sum_similar_dw;
      }
      else{
        //dissimilar pairs
        Dtype dist = sqrt(auxiLayer->dist_sq_.cpu_data()[i]);
...
#ifdef CPU_ONLY
STUB_GPU(DistanceWeightedValidPairLossV5Layer);
#endif

INSTANTIATE_CLASS(DistanceWeightedValidPairLossV5Layer);
REGISTER_LAYER_CLASS(DistanceWeightedValidPairLossV5);
