...
void WeightedSoftmaxWithLossLayer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < outer_num_; ++i) {
    for (int j = 0; j < inner_num_; j++) {
      ...
      /*
      // Noisy images
      if( prob_data[i * dim + label_value * inner_num_ + j] - threshold < Dtype(1e-12) ){
        loss -= 0;
      }
      else{
  ...
  //top[0]->mutable_cpu_data()[0] = loss / ( Dtype(count) + Dtype(1e-6) );
...
void WeightedSoftmaxWithLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    for (int i = 0; i < outer_num_; ++i) {
      for (int j = 0; j < inner_num_; ++j) {
        ...
        if (has_ignore_label_ && label_value == ignore_label_) {
          ...
          //To remove some extreme noisy images for gradient backpropagation
          //Actually, the same to ignore this instance. 
          if( bottom_diff[i * dim + label_value * inner_num_ + j] - threshold < Dtype(1e-12) ){
            for (int c = 0; c < bottom[0]->shape(softmax_axis_); ++c) {
              bottom_diff[i * dim + c * inner_num_ + j] = 0;
            }
          }
    ...
    //Dtype loss_weight = top[0]->cpu_diff()[0] / ( Dtype(count) + Dtype(1e-6) );
