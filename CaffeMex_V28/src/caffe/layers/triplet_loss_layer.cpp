...
namespace caffe {
  ...
  void TripletLossLayer<Dtype>::Forward_cpu( const vector<Blob<Dtype>*>& bottom, 
    ...
    if( bottom.size() > 3 && 
        ( (bottom[3]->cpu_data()[0] - threshold < Dtype(1e-12) || bottom[4]->cpu_data()[0] - threshold < Dtype(1e-12)) || 
            bottom[5]->cpu_data()[0] - threshold < Dtype(1e-12) 
        ) 
      )
    {
      top[0]->mutable_cpu_data()[0] = Dtype(0);

      LOG(INFO) << bottom[3]->cpu_data()[0] << " " << bottom[4]->cpu_data()[0] << " " << bottom[5]->cpu_data()[0];
      LOG(INFO) << "abnormal tracklet " ;

      //prepare for backward pass, actually do not need to change the Backward_cpu function
      caffe_set(diff_ap_.count(), Dtype(0), diff_ap_.mutable_cpu_data() );
      caffe_set(diff_an_.count(), Dtype(0), diff_an_.mutable_cpu_data() );
      caffe_set(diff_pn_.count(), Dtype(0), diff_pn_.mutable_cpu_data() );
    ...
    for (int i = 0; i < bottom[0]->num(); ++i) {
      ...
      //LOG(INFO) << "mdist" << mdist << " ap: " <<dist_sq_ap_.mutable_cpu_data()[i] << " an: " << dist_sq_an_.mutable_cpu_data()[i];
