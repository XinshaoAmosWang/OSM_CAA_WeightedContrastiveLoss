...
tracklet_num = 8298;
train_tracklets = cell(tracklet_num,1);
train_labels = zeros(tracklet_num,1);
train_cameras = zeros(tracklet_num,1);
...
%for dir_index = 4 : 4
    ...
    for i = 1 : length(tracklet_ids_uni)
        ...
        train_tracklets{count} = extract_tracklet(cur_dir,image_names, inds);
        train_labels(count) = dir_index-3;
        train_cameras(count) = cam_ids( inds(1) );
