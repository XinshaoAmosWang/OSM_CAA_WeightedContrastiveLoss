function [tracklet_feature] = process_tracklet_LSTM_last_sub(net, tracklet, batch_size)
...
%%%%select sub tracklet
if(length(tracklet) >= batch_size)
    sample = randperm(length(tracklet), batch_size);
else
    sample = randperm(length(tracklet), length(tracklet));
    for i = length(tracklet) + 1 : batch_size
        sample(i) = randperm(length(tracklet), 1);
    end
end
sample = sort(sample);
tracklet = tracklet(sample);
